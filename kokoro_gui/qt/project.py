...
def extract_small(info: BundleInfo, project_dir: str) -> None:
    ...
    with zipfile.ZipFile(info.path) as zf:
        for entry in info.entries:
            if entry.filename.replace("\\", "/").startswith("audio/"):
                continue
...
def plan_save(document: Document, project_settings: dict, path: str, project_dir: str, project_id: str,
    ...
    def to_relative(abs_path):
        if not os.path.isabs(abs_path) or not os.path.isfile(abs_path):
            return abs_path.replace("\\", "/")
        real = os.path.realpath(abs_path)
        if project_root and real.startswith(project_root + os.sep):
            rel = os.path.relpath(real, project_root).replace("\\", "/")
        else:
            # A file outside the project dir (a migrated .json's audio_output
            # segment): bundled under audio/generated by its own name.
            rel = f"{AUDIO_GENERATED}/{os.path.basename(real)}"
        if rel not in seen:
            seen.add(rel)
